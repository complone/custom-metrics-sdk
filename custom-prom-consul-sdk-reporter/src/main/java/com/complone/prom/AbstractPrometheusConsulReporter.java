...
public class AbstractPrometheusConsulReporter implements MetricReporter{
    ...
    private Set<String> labelFilterKeySet = new HashSet<>();

    @Override
    public void open(MetricConfig config) {
        boolean filterLabelValueCharacters = config.getBoolean(
                PrometheusPushGatewayConsulReporterOptions.FILTER_LABEL_VALUE_CHARACTER.key(), PrometheusPushGatewayConsulReporterOptions.FILTER_LABEL_VALUE_CHARACTER.defaultValue());

        if (!filterLabelValueCharacters) {
            labelValueCharactersFilter = input -> input;
        }

        String filterLabelKeys = config.getString(PrometheusPushGatewayConsulReporterOptions.FILTER_LABEL_KEY.key(), PrometheusPushGatewayConsulReporterOptions.FILTER_LABEL_KEY.defaultValue());
        labelFilterKeySet.addAll(Arrays.asList(filterLabelKeys.split(",")));
    ...
    public void notifyOfAddedMetric(final Metric metric, final String metricName, final MetricGroup group) {
        ...
        for (final Map.Entry<String, String> dimension : group.getAllVariables().entrySet()) {
            ...
            if (labelFilterKeySet.contains(key.substring(1, key.length() - 1))) {
                continue;
            }
